...
private struct Reaction: Identifiable, Hashable {
    let id: String
    let height: CGFloat
}
...
struct FLForEachTests {
    ...
    private func reactions(_ ids: [String]) -> [Reaction] {
        ids.map { Reaction(id: $0, height: 20) }
    }
    ...
    func spacerInsideForEachStillWorks() {
        let node = FLVStack(spacing: 0) {
            ...
            FLForEach([Reaction(id: "gap", height: 0)]) { _ in
                FLSpacer()
            }
