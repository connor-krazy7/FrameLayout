...
final class FLViewRegistry {
    ...
    func view<Tag: Hashable>(withTag tag: Tag) -> UIView? {
        tagToView[AnyHashable(tag)]
    }

    func button<Tag: Hashable>(withTag tag: Tag) -> UIControl? {
        view(withTag: tag) as? UIControl
    }

    func view<Tag: Hashable, Kind: UIView>(withTag tag: Tag, as kind: Kind.Type) -> Kind? {
        guard let tagged = view(withTag: tag) else { return nil }

        return tagged as? Kind ?? Self.firstDescendant(of: kind, in: tagged)
    }

    func imageView<Tag: Hashable>(withTag tag: Tag) -> UIImageView? {
        view(withTag: tag, as: UIImageView.self)
    }

    func label<Tag: Hashable>(withTag tag: Tag) -> UILabel? {
        view(withTag: tag, as: UILabel.self)
    }

    func containsView<Tag: Hashable>(withTag tag: Tag) -> Bool {
        tagToView[AnyHashable(tag)] != nil
    }

    func bindView<Tag: Hashable>(
        withTag tag: Tag,
    ...
    func bindView<Tag: Hashable, Key: Hashable>(
        withTag tag: Tag,
        bindingKey: Key,
        _ configure: @escaping @MainActor (UIView) -> Void
    ) {
    ...
    func bindButton<Tag: Hashable, Key: Hashable>(
        withTag tag: Tag,
        bindingKey: Key,
        _ configure: @escaping @MainActor (UIControl) -> Void
    ) {
    ...
    func bindAction<Tag: Hashable, Key: Hashable>(
        withTag tag: Tag,
        for events: UIControl.Event = .touchUpInside,
        bindingKey: Key,
        _ handler: @escaping @MainActor (UIControl.Event) -> Void
    ...
    func unbindView<Tag: Hashable>(withTag tag: Tag) {
        unbind(tag: AnyHashable(tag), bindingKey: Constants.BindingKey.defaultKey)
    }

    func unbindView<Tag: Hashable, Key: Hashable>(withTag tag: Tag, bindingKey: Key) {
        unbind(tag: AnyHashable(tag), bindingKey: AnyHashable(bindingKey))
    }

    func unbindButton<Tag: Hashable>(withTag tag: Tag) {
        unbindView(withTag: tag)
    }

    func unbindButton<Tag: Hashable, Key: Hashable>(withTag tag: Tag, bindingKey: Key) {
        unbindView(withTag: tag, bindingKey: bindingKey)
    }

    func unbindAction<Tag: Hashable>(withTag tag: Tag) {
        unbindAction(tag: AnyHashable(tag), bindingKey: Constants.BindingKey.defaultActionKey)
    }

    func unbindAction<Tag: Hashable, Key: Hashable>(withTag tag: Tag, bindingKey: Key) {
        unbindAction(tag: AnyHashable(tag), bindingKey: AnyHashable(bindingKey))
    }

    func unbindAll<Tag: Hashable>(withTag tag: Tag) {
    ...
    func registerView<Tag: Hashable>(_ view: UIView, withTag tag: Tag) {
