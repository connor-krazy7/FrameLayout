...
struct FLTextLayout: FLLayout {
    let size: CGSize
}

struct FLText: FLNode {
    typealias View = FLTextView

    static var typeIdentifier: String { "text" }

    /// Matches `UILabel`'s own defaults, so an unstyled `FLText` renders the same as an unstyled label.
    static var defaultFont: UIFont { .systemFont(ofSize: UIFont.labelFontSize) }
    static var defaultColor: UIColor { .label }
    ...
    func resolvedText(in environment: FLEnvironment) -> NSAttributedString {
...
final class FLTextView: UILabel, FLNodeView {
    typealias Node = FLText
