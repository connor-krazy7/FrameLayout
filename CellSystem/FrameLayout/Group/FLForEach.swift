...
struct FLForEachItem<ID: Hashable & Sendable, Item: FLNode>: Sendable, Hashable {
    let id: ID
    let node: Item
}

struct FLForEach<ID: Hashable & Sendable, Item: FLNode>: FLGroup {
    typealias Views = FLForEachViews<ID, Item>

    static var typeIdentifier: String { "forEach(\(Item.typeIdentifier))" }

    let items: [FLForEachItem<ID, Item>]

    var childCount: Int { items.count }

    init(items: [FLForEachItem<ID, Item>]) {
        self.items = items
    }
...
@MainActor
final class FLForEachViews<ID: Hashable & Sendable, Item: FLNode>: FLGroupViews {
    typealias Group = FLForEach<ID, Item>
