...
struct FLImageComparisonTests {
    private let box = FixtureSwatch.boxWidth
    ...
    func intrinsicIgnoresTheProposal() {
        let landscape = FixtureSwatch.landscape
    ...
    func resizableTakesTheProposal() {
        let reserved = size(FLImage(FixtureSwatch.landscape).resizable())
    ...
    func fillOverflowsOnlyWithASuppliedHeight() {
        ...
        let image = FLImage(FixtureSwatch.landscape).resizable()
    ...
    func fixedFrameContainsAFill() {
        let reserved = size(
            FLImage(FixtureSwatch.landscape)
                .resizable()
    ...
    func contentModeIsLayoutNeutral() {
        let portrait = FLImage(FixtureSwatch.portrait).resizable()
    ...
    func smallImageDependsOnTheCap() {
        let tiny = FLImage(FixtureSwatch.tiny).resizable().aspectRatio(4.0 / 3, contentMode: .fit)
